...
import api from '../api/axios'; // ✅ Remplacé Firebase par Axios
...
function PropertyDetail() {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      setLoading(true);
      try {
        // ✅ Récupération via ton API Node.js
  ...
  // ✅ Gestion des images (MongoDB Atlas)
  ...
  if (error) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white p-6 text-center">
      ...
      <button onClick={() => navigate(-1)} className="bg-slate-900 text-white px-8 py-3 rounded-2xl font-bold transition-transform active:scale-95">
    ...
    <div className="min-h-screen bg-[#FDFDFD] pt-24 pb-20">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-slate-500 hover:text-slate-900 transition-colors font-bold group">
          ...
          <div className="lg:col-span-8 space-y-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-6 border-b border-slate-100 pb-10">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <span className="bg-emerald-50 text-emerald-600 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-emerald-100">
                    {property.typeSejour === 'court' ? 'Court Séjour' : 'À Louer'}
                  </span>
              ...
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm min-w-[200px] text-center">
                <p className="text-slate-400 text-xs font-black uppercase tracking-widest mb-1">
                  {property.typeSejour === 'court' ? 'Par jour' : 'Prix mensuel'}
                </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                { label: 'SdB', value: property.bathrooms || '0', icon: Bath },
                { label: 'Surface', value: `${property.surface || '--'} m²`, icon: Square },
                { label: 'Type', value: property.typeSejour === 'court' ? 'Studio/Appart' : 'Villa/Maison', icon: Calendar },
              ].map((stat, i) => (
                <div key={i} className="bg-white p-5 rounded-[2rem] border border-slate-100 flex flex-col items-center gap-2 hover:border-emerald-100 transition-colors">
            ...
            <div className="bg-white p-8 md:p-10 rounded-[3rem] border border-slate-100">
              ...
              <p className="text-slate-600 leading-relaxed text-lg font-medium italic opacity-80">
                "{property.description || "Un espace exceptionnel situé à Abidjan, offrant confort et modernité pour votre séjour."}"
              </p>
            </div>
          </div>
              ...
              <div className="bg-slate-950 p-8 rounded-[3rem] text-white shadow-2xl shadow-slate-200 overflow-hidden relative">
                <div className="relative z-10">
                  ...
                  <div className="flex items-center gap-4 mb-8">
                    <div className="w-16 h-16 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl flex items-center justify-center text-2xl font-black">
                  ...
                  <div className="space-y-4">
                    <a href={`tel:${property.agentPhone}`} className="flex items-center justify-between w-full bg-white/5 hover:bg-white/10 p-4 rounded-2xl border border-white/10 transition-all group">
                      <div className="flex items-center gap-3">
                        <Phone size={18} className="text-emerald-400" />
                        <span className="text-sm font-bold">{property.agentPhone || "Contact non disponible"}</span>
                      </div>
                      <ChevronRight size={16} className="text-white/30 group-hover:translate-x-1 transition-transform" />
                    </a>
                    <a href={`mailto:${property.agentEmail}`} className="flex items-center justify-between w-full bg-white/5 hover:bg-white/10 p-4 rounded-2xl border border-white/10 transition-all group">
                      <div className="flex items-center gap-3">
              ...
              <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-xs font-medium leading-relaxed">
                    Ne payez jamais d'acompte avant d'avoir visité le bien avec un agent certifié Betna.
                  </p>
                </div>
              </div>
